...
def job_config = [
        job: [
                name: "TesteBefore123_Pipeline_develop"
        ],
        git: [
                branch: "develop"
        ]
...
def customLib = library identifier: 'TesteBefore123_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/TesteBefore123_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.TesteBefore123.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "TesteBefore123_Pipeline_develop", "develop")
                }
            }    
        }
        stage('Build') {
